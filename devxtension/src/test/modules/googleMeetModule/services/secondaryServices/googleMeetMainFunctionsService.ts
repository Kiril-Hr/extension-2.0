...
import {GoogleMeetMainFunctionsInterface} from "../../types/googleMeetInterfaces";
...
export class GoogleMeetMainFunctionsService implements GoogleMeetMainFunctionsInterface {
    ...
    constructor(
      ...
      public DateService = dependencyContainer.getService<typeof GoogleMeetDateService>(GoogleMeetDateService.name),
    ){}
    ...
    public getMeetTopic = (meetingsData): currentMeetingTitleType => {
        ...
        for (let i = 0; i < startTimes.length; i++) {
          ...
          if ((localeCurrentDate >= currentTime && (localeCurrentDate < nextTime || !nextTime))) {
            const time = this.UtilsService.convertTimeWithZero(currentTime as Date)
